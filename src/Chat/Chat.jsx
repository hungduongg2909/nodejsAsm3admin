...
function Chat(props) {
   ...
   useEffect(() => {
      ...
      socket.on("room_ended", (endedRoomId) => {
         // Xử lý UI ở đây, ví dụ gỡ khỏi danh sách hoặc đánh dấu đã kết thúc
         console.log("Room ended:", endedRoomId);

         // Nếu admin đang mở room đó thì có thể clear luôn
         if (roomId === endedRoomId) {
            setRoomId("");
            setMessage([]);
         }

         // Hoặc filter khỏi allRoom nếu muốn
         setAllRoom((prev) => prev.filter((r) => r._id !== endedRoomId));
      });
